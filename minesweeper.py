...
class Sentence():
    ...
    def __init__(self, cells, count, mines=None, safes=None):
        self.cells = set(cells)
        self.count = count
        self.mines = mines if mines is not None else set()
        self.safes = safes if safes is not None else set()
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # Add a new sentence based on the count of neighboring mines
        if neighbors:
            new_sentence = Sentence(neighbors, count)
            self.knowledge.append(new_sentence)
    ...
    def evaluate_safe_moves(self):
        potential_moves = set()
        for sentence in self.knowledge:
            if len(sentence.cells) == sentence.count:
                potential_moves.update(sentence.cells)

        for move in potential_moves:
            if move not in self.mines:
                self.safes.add(move)

        print(f"Sigurni potezi nakon evaluacije: {self.safes}")
